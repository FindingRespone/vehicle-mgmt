      ...
      {vehicles.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {search || statusFilter || availabilityFilter ? '未找到匹配的车辆' : '暂无车辆数据'}
          </h3>
          ...
          {totalPages > 1 && (
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:hidden">
                {page > 1 ? (
                  <Link
                    href={`/vehicles?page=${page - 1}${search ? `&search=${search}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}${availabilityFilter ? `&availability=${availabilityFilter}` : ''}`}
                  ...
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return (
                        <Link
                          key={pageNum}
                          href={`/vehicles?page=${pageNum}${search ? `&search=${search}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}${availabilityFilter ? `&availability=${availabilityFilter}` : ''}`}
